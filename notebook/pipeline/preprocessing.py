...
def do_preprocessing(data_file):
    data = pd.read_csv(f'/opt/ml/processing/input/{data_file}')
    ...
    test_data.to_csv(test_features_output_path, index = False, header = True)

if __name__=='__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--data-file", type=str, default = "bank-additional-full.csv")
    args, _ = parser.parse_known_args()

    print("Received arguments {}".format(args))
    data_file = args.data_file
    
    do_preprocessing(data_file)   
