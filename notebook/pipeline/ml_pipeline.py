...
def create_endpoint_step(execution_input):
    ...
    return endpoint_creation_step

def update_endpoint_step(execution_input):
...
def create_workflow(
    ...
    hpo_job_sns_notification_step = create_hpo_job_sns_notification_step()
    ...
    endpoint_creation_step = create_endpoint_step(execution_input)
    endpoint_update_step = update_endpoint_step(execution_input)
    ...
    query_endpoint_deployment_lambda_step.next(deployed_endpoint_updating_step)
    ...
    to_do_hpo_choice_step = create_to_do_hpo_choice_step(tuning_step, to_do_training_choice_step)

    input_code_uri = upload_preprocess_code(bucket_name)
    processing_step = create_preprocessing_step(execution_input, bucket_name, input_code_uri, data_file, experiment_name)
